...
Paths:
 - GET  /api/flagged-artists           -> list all flagged artists for current user
...
def list_flagged(owner: str = Depends(get_current_user)):
    """
    Return list of flagged artists for the current user.
    Protected endpoint.
    """
    return fa.list_flagged_artists(owner)
...
def add_flagged(payload: Dict = Body(...), owner: str = Depends(get_current_user)):
    """
    Add a flagged artist for the current user.
    Payload should be {"name": "<Exact Artist Name>"}.
...
def delete_flagged(artist_id: int, owner: str = Depends(get_current_user)):
    """
    Delete a flagged artist by id for the current user.
    """
    ok = fa.delete_flagged_artist(artist_id, owner)
    if not ok:
